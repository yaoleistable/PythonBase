...
'''def interpolation(x):  # 插值法求年径流总量控制率
    y1, y2 = 60, 70  # 年径流总量控制率
    x1, x2 = 23.1, 31.3  # 设计降雨量
    y = (y2-y1)*(x-x1)/(x2-x1)+y1
    return print("{}mm降雨量对应年径流总量控制率为:{:.2f}%".format(x, y))


def interpolation_y(x):  # 插值法求降雨量
    y1, y2 = 23.1, 31.3   # 设计降雨量
    x1, x2 = 60, 70  # 年径流总量控制率
    y = (y2-y1)*(x-x1)/(x2-x1)+y1
    return print("年径流总量控制率为{}%对应降雨量为:{:.2f}mm".format(x, y))'''
